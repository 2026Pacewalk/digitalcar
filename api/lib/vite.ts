...
async function cardSnapshotMeta(pathname: string): Promise<CardMeta | null> {
  ...
  try {
    ...
    if (cust) {
      ...
      const image = `${SITE}/og/${encodeURIComponent(slug)}.png?v=${ogVersion([s(cust.name) || slug, s(cust.company_name), s(cust.designation), logo, cust.color, cust.color2, cust.photo])}`;
      const url = `${SITE}/${slug}`;
