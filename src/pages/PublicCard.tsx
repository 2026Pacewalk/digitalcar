...
/* A first-party, random, non-personal visitor key.

   It is generated by the visitor's own browser, never leaves this origin, and is
   hashed again with a server secret before it is stored — so it supports
   "unique vs returning visitors" reporting without identifying anybody. Storage
   may be unavailable (private mode / blocked); we degrade to no id rather than
   throwing, and the event is still counted, just not de-duplicated. */
...
export default function PublicCard({ slugOverride }: { slugOverride?: string } = {}) {
  ...
  useEffect(() => {
    const onEnquiry = (e: MessageEvent) => {
      ...
      // Analytics event relayed from the sandboxed card (views, taps, section
      // views, dwell time…). The server validates `type` against its own
      ...
      if (data && data.__dcTrack) {
        try {
          ...
          fetch("/api/track", {
            ...
            body: JSON.stringify({
              ...payload,
              slug,                                   // always OUR slug, never the frame's
