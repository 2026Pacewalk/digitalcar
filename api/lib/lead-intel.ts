...
// An e-mail address is contact information, not a promotional link. Without
// this, a visitor who writes "send the price to me@gmail.com" scored as spam.
...
export function classifyLeadRules(l: LeadLike): LeadVerdict {
  ...
  // A lead that came from a CARD is a visitor who deliberately opened this
  // person's card and filled in their form, so a single signal is weak evidence
  // there — especially as the keyword list necessarily contains the industries
  // our own customers work in (loan, SEO, digital marketing, investment). One
  // signal alone no longer suppresses a card lead; two still do. Website-form
  // leads (where the real spam arrives) keep the original weights.
  const sig = card ? 2 : 3;
  const linkHay = hay.replace(EMAIL_RE, " ");
  if (URL_RE.test(linkHay)) { spam += sig; reasons.push("contains a link"); }
  if (SPAM_KW.test(hay)) { spam += sig; reasons.push("scam / marketing keywords"); }
