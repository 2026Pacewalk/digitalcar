...
import { Share2, Save, Trash2, Palette, Plus } from "lucide-react";
          ...
          {links.length === 0 ? (
            ...
            <div className="space-y-2.5">
              {links.map((l, i) => {
                ...
                if (!p) return null;
                return (
                  <div key={i} className="flex items-center gap-3 rounded-xl border border-[#F1F5F9] bg-white p-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] font-semibold text-[#334155] mb-0.5">{p.key === "website" ? "Custom link" : p.label}</p>
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
              {available.map((p) => (
                <button key={p.key} type="button" onClick={() => addPlatform(p.key)}
                  className="flex flex-col items-center gap-1.5 rounded-xl border border-[#E2E8F0] bg-white p-2.5 hover:border-[#F7B31C] hover:-translate-y-0.5 transition-all">
                  <span className="w-9 h-9 rounded-full flex items-center justify-center" style={{ background: p.color, color: p.fg || "#fff", boxShadow: "0 2px 6px rgba(15,23,42,.18)" }}><PlatformIcon p={p} size={17} /></span>
