...
async function seed() {
  ...
  // ─── Seed Super Admin ───
  // Email/password come from env (set them in production!). Fall back only for
  // local dev convenience. Re-seeding updates the password.
  const adminEmail = (process.env.ADMIN_EMAIL || "hellopacewalk@gmail.com").toLowerCase();
  ...
  const [adminResult] = await db.insert(users).values({
    ...
    role: "super_admin",
    status: "active",
  }).onDuplicateKeyUpdate({ set: { password: adminPass, role: "super_admin" } });
  console.log(`Super admin seeded: ${adminEmail} (password ${process.env.ADMIN_PASSWORD ? "from ADMIN_PASSWORD" : "= admin123 [dev default]"})`);
  ...
  console.log("\n===== DEMO CREDENTIALS =====");
  console.log(`Admin:    ${adminEmail}    (see ADMIN_PASSWORD)`);
