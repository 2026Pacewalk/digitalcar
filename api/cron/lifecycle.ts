...
export async function runLifecycle(): Promise<{ enabled: boolean; scanned: number; sent: number; abandoned: number }> {
  ...
  const slugRows = uids.length ? await db.select({ userId: publishedCards.userId, slug: publishedCards.slug }).from(publishedCards).where(inArray(publishedCards.userId, uids)) : [];
