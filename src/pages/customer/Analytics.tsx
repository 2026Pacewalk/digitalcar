...
import { Eye, MessageSquare, BarChart3, QrCode, Phone, MessageCircle, Mail, Globe, MapPin, UserPlus } from "lucide-react";
...
export default function CustomerAnalytics() {
  ...
  // Real, live Total Views — same source as the public card + dashboard (base +
  // tracked views), so all three agree instead of the near-empty legacy table.
  ...
  useEffect(() => {
    ...
    fetch(`/api/views/${encodeURIComponent(slug)}`).then((r) => r.json())
      ...
      .catch(() => { /* fall back to the query value */ });
  ...
  // Headline shows the real, live Total Views (matches the card + dashboard).
  // Unique Visitors / Cards are dropped here because the per-visitor table isn't
  // recorded for legacy/imported cards, so they'd read a misleading 0.
  const stats = [
    { label: "Total Views", value: realViews ?? (data?.totalViews ?? 0), icon: Eye, color: "bg-[#FEF3C7] text-[#92400E]" },
  ...
  // The daily chart, traffic sources and devices come from the per-visitor table,
  // which legacy/imported cards don't populate — hide those panels when empty
  // rather than showing "no data" next to a real view count.
  const hasRichAnalytics = chart.length > 0 || sources.length > 0 || devices.length > 0;
  ...
  return (
    <ResponsiveDashboardLayout>
      ...
      <div className="p-6 space-y-6">
        ...
        {/* Per-card performance (real) — only when there are relational cards */}
        {cards.length > 0 && (
