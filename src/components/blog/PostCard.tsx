...
import { ShareIconButton } from "./ShareMenu";
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-3xl bg-white ring-1 ring-[#E9EDF3] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_24px_60px_-28px_rgba(15,23,42,0.35)] hover:ring-[#F7B31C]/60">
      <div className="relative aspect-[16/9] overflow-hidden">
        ...
        {/* Above the card-wide link overlay, so it shares instead of opening the article. */}
        <ShareIconButton post={post} className="absolute right-4 top-4 z-10" />
      </div>
      <div className="flex flex-1 flex-col p-6">
        ...
        <div className="mt-auto flex items-center justify-between pt-5 text-[12.5px] text-[#94A3B8]">
          <span className="inline-flex items-center gap-1.5"><Clock size={13} aria-hidden="true" /> {readingMinutes(post)} min read · {formatBlogDate(post.publishedAt)}</span>
