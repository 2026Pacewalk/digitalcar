...
export default function AdminCards() {
  ...
  return (
    <DashboardLayout>
      <TopBar title="Cards" subtitle="All digital cards on the platform" />
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-[#F1F5F9]">
                {isLoading ? (
                  ...
                  <tr><td colSpan={6} className="px-6 py-12 text-center text-sm text-[#94A3B8]">No cards found</td></tr>
                ) : (
                  cards.map((card: Record<string, unknown>) => (
                    <tr key={card.id as number} className="hover:bg-[#F8FAFC] transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-[#64748B]">{card.ownerName as string || "—"}</td>
                      <td className="px-6 py-4">
